...
class TestDockerImage(unittest.TestCase):
    ...
    def test_data_loaded(self):
        ...
        cur.execute("select matviewname from pg_matviews where pg_matviews.schemaname = 'public'")
        ...
        self.assertEquals(tables, ['raw_confirmed', 'raw_death', 'raw_recovered'])
        self.assertEquals(views, ['countries_combined', 'countries_combined_now', 'countries_confirmed', 'countries_death', 'countries_recovered', 'i100_countries', 'latest_date'])

        self.assertEquals(requests.get("http://localhost:3000/api/util/stats").json()["stats"]["database"]["databases"]["total"], 1)
